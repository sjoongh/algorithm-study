...
def solution(n, m, section):
    # 칠해야할 곳이 연속되어 있어도 m의 길이에 따라 제한이 되어야함
    # section이 연속되어 있고 m보다 작다면 한번에 칠할 수 있음
    # n이 m 이하라면 한번에 칠할 수 있으므로 무조건 1임
    answer = 1
    paint = section[0]
    for i in range(1, len(section)):
        # section[i] - section[i-1] 이 m보다 크거나 같다면
        # 칠할 수 있는 범위이므로 answer += 1
        # 만약 m이 더 작다면 더 칠할 수 있으므로 continue
